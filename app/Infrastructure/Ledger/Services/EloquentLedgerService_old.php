...
final class EloquentLedgerService_old implements LedgerService
{
    public function postDepositCredit(
        ...
        DB::transaction(function () use ($depositId, $operationId, $metadata): void {
            ...
            $account = EloquentAccount::query()
                ->where('user_id', $deposit->user_id) //denis  user_id -нет!!!!!! ->>>owner_id owner_type
...
//            }
             ...
             */
            $reverseOperationId = 'deposit-reversal:' . $deposit->id;//проще для idempotency
            //$reverseOperationId = 'deposit-reversal:' . $deposit->credited_operation_id; // лучше связывает reversal с исходной операцией.
